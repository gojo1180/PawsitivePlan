...
export default function PetTab({ data, onClickShop, onFeedPet, isFeeding, onDeletePet, onRefresh }: PetTabProps) {
  ...
  return (
    <div className="flex-1 w-full h-full relative overflow-hidden bg-gradient-to-b from-base to-crust">
      ...
      <div
        className={`absolute inset-0 z-10 w-full h-full pb-16 transition-all duration-300 ${
          isDraggingOver ? "bg-mauve/10 backdrop-blur-[2px]" : ""
        }`}
        onDragOver={(e) => {
          e.preventDefault();
      ...
      <div className="absolute inset-0 z-20 pointer-events-none p-6 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start pointer-events-auto w-full">
          <div className="bg-surface0/80 backdrop-blur-md rounded-2xl border-2 border-surface1 p-4 shadow-xl w-[260px] hover:border-surface2 transition-colors">
            <div className="flex justify-between items-center mb-3 border-b-2 border-surface1 pb-2">
              <div>
                <h2 className="font-black text-mauve text-lg leading-tight truncate">{optimisticPet.name || "My Pet"}</h2>
                <p className="text-[10px] text-subtext0 font-black uppercase tracking-widest">{optimisticPet.species}</p>
              </div>
          ...
          <button
            ...
            className={`backdrop-blur-md text-text border-2 p-4 rounded-3xl shadow-xl flex items-center gap-3 transition-all hover:-translate-y-1 group
          ...
          >
            ...
            <div className="text-left hidden md:block px-1">
              <p className="text-xs font-black uppercase tracking-wider text-subtext0 leading-tight">Gudang</p>
        ...
        <div className="flex justify-end items-end pointer-events-auto">
          <div
            id="trash-can"
            className="w-20 h-20 bg-red/10 border-4 border-dashed border-red/40 text-red rounded-3xl flex flex-col items-center justify-center transition-all shadow-lg opacity-40 hover:opacity-100 hover:scale-110 hover:bg-red/20 hover:border-red cursor-pointer"
      ...
      <AnimatePresence>
        {showInventory && (
          <motion.div
            ...
            className="absolute top-[120px] right-6 w-[420px] max-h-[68%] bg-surface0/97 backdrop-blur-xl border-2 border-surface2 rounded-3xl shadow-2xl z-30 flex flex-col pointer-events-auto overflow-hidden"
          >
            {/* Header */}
            <div className="flex justify-between items-center p-5 border-b-2 border-surface1 bg-surface0/50 shrink-0">
              <h3 className="font-black text-lg text-text flex items-center gap-2">
                <Backpack size={22} className="text-mauve" /> Inventory Barang
              </h3>
              <div className="flex items-center gap-2">
                <span className="text-[10px] bg-mauve/20 text-mauve px-2 py-1 rounded-full font-black">
                  Tarik ke Pet untuk pakai
                </span>
                <button
                  onClick={() => setShowInventory(false)}
                  className="text-subtext0 hover:text-red hover:bg-red/10 p-2 rounded-full transition-colors"
                >
                    ...
                    {draggableItems.map((item) => {
                      ...
                      <div
                        ...
                        <motion.div
                          ...
                          className={`bg-base/80 border-2 rounded-2xl p-3 flex flex-col items-center justify-between text-center relative group shadow-sm transition-all cursor-grab active:cursor-grabbing select-none
                            ${item.is_equipped ? `ring-2 ring-${accentColor} border-${accentColor}` : "border-surface1"}
                            hover:border-${accentColor} hover:shadow-md hover:-translate-y-0.5`}
                        >
                          <div className={`w-16 h-16 flex items-center justify-center mb-2 mt-1 rounded-xl bg-surface1/40 transition-colors group-hover:bg-${accentColor}/10`}>
                            <img
                          ...
                          {isConsumable ? (
                            <button
                              ...
                              className="w-full bg-green text-crust font-black text-xs py-1.5 rounded-xl border-b-[3px] border-teal active:translate-y-px active:border-b-0 disabled:opacity-40 transition-all uppercase tracking-wider"
                            >
                              {isFeeding ? "..." : "Beri"}
                            </button>
                          ) : (
                            <div className={`w-full text-center text-[10px] font-black py-1.5 rounded-xl uppercase tracking-wider ${item.is_equipped ? "bg-mauve/20 text-mauve" : "bg-surface1 text-subtext0"}`}>
                              {item.is_equipped ? "Dipakai" : "Tarik ke pet"}
                            </div>
                          )}

                          {/* Quantity badge */}
                          <span className="absolute -top-3 -right-3 text-[11px] font-black text-crust bg-yellow w-7 h-7 flex items-center justify-center rounded-full border-2 border-surface0 shadow-sm z-10">
                            {item.quantity}
                          </span>
                        </motion.div>
                        </div>
